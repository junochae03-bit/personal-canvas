...
// ────────────── comic ──────────────
test('LAYOUTS: 모든 패널이 페이지 범위 안에 들어감', () => {
  for(const layout of LAYOUTS){
    assert.ok(layout.id && layout.name && layout.w > 0 && layout.h > 0, `${layout.id}: 기본 필드 누락`);
    // NAI 무료 한도 1MP 검증
    assert.ok(layout.w * layout.h <= 1024 * 1216 + 100, `${layout.id}: 1MP 초과 (${layout.w}×${layout.h})`);
    for(const p of layout.panels){
      assert.ok(p.x >= 0 && p.y >= 0, `${layout.id}: 음수 좌표`);
      assert.ok(p.x + p.w <= layout.w, `${layout.id}: 패널 width 초과 (${p.x}+${p.w} > ${layout.w})`);
      assert.ok(p.y + p.h <= layout.h, `${layout.id}: 패널 height 초과 (${p.y}+${p.h} > ${layout.h})`);
      // 인페인트 디테일 최소치 — 너무 작은 패널은 의미 없음
      assert.ok(p.w >= 200 && p.h >= 200, `${layout.id}: 패널 크기 200px 미만 (NAI 디테일 손실)`);
    }
  }
});

test('LAYOUTS: ID 중복 없음', () => {
  const ids = LAYOUTS.map(l => l.id);
  assert.equal(ids.length, new Set(ids).size);
});

test('getLayout: ID 매칭', () => {
  assert.equal(getLayout('4grid').name, '4컷 그리드');
  assert.equal(getLayout('2v').panels.length, 2);
  assert.equal(getLayout('invalid'), null);
  assert.equal(getLayout(null), null);
  assert.equal(getLayout(''), null);
});

test('getPanelMaskCoords: 패널 인덱스 → 좌표', () => {
  const layout = getLayout('4grid');
  assert.deepEqual(getPanelMaskCoords(layout, 0), {x: 0, y: 0, w: 506, h: 506});
  assert.deepEqual(getPanelMaskCoords(layout, 3), {x: 518, y: 518, w: 506, h: 506});
  assert.equal(getPanelMaskCoords(layout, 99), null);
  assert.equal(getPanelMaskCoords(layout, -1), null);
  assert.equal(getPanelMaskCoords(null, 0), null);
});

test('wrapBubbleText: 단어 경계 워드랩', () => {
  assert.deepEqual(wrapBubbleText('hello world', 5), ['hello', 'world']);
  assert.deepEqual(wrapBubbleText('hello world', 12), ['hello world']);
  assert.deepEqual(wrapBubbleText('a b c d e', 3), ['a b', 'c d', 'e']);
});

test('wrapBubbleText: 긴 단어 강제 분할', () => {
  assert.deepEqual(wrapBubbleText('supercalifragilistic', 6),
    ['superc', 'alifra', 'gilist', 'ic']);
});

test('wrapBubbleText: 한국어 (공백 없음 → 글자 단위)', () => {
  assert.deepEqual(wrapBubbleText('안녕하세요반갑습니다', 4),
    ['안녕하세', '요반갑습', '니다']);
});

test('wrapBubbleText: 줄바꿈 보존', () => {
  assert.deepEqual(wrapBubbleText('a\nb', 10), ['a', 'b']);
  assert.deepEqual(wrapBubbleText('hello\nworld', 10), ['hello', 'world']);
});

test('wrapBubbleText: 빈 입력 안전', () => {
  assert.deepEqual(wrapBubbleText('', 5), []);
  assert.deepEqual(wrapBubbleText(null, 5), []);
});

test('bubbleSvgString: 도형별 SVG 구분', () => {
  const round = bubbleSvgString({shape:'round', x:10, y:20, w:100, h:60, text:'hi'});
  assert.match(round, /<ellipse[^>]*stroke="black"/);
  assert.ok(!/stroke-dasharray/.test(round));
  assert.match(round, />hi</);

  const spike = bubbleSvgString({shape:'spike', x:0, y:0, w:100, h:60, text:'BANG'});
  assert.match(spike, /<polygon/);

  const thought = bubbleSvgString({shape:'thought', x:0, y:0, w:100, h:60, text:'...'});
  assert.match(thought, /stroke-dasharray/);
});

test('bubbleSvgString: 텍스트 XML escape (XSS 방어)', () => {
  const out = bubbleSvgString({shape:'round', x:0, y:0, w:200, h:80, text:'<script>alert(1)</script>'});
  assert.ok(out.includes('&lt;script&gt;'));
  assert.ok(!out.includes('<script>'));
});

test('bubbleSvgString: 잘못된 shape → round 폴백', () => {
  const out = bubbleSvgString({shape:'invalid', x:0, y:0, w:100, h:60, text:'x'});
  assert.match(out, /data-shape="round"/);
});

test('bubbleSvgString: 최소 크기 보정', () => {
  const out = bubbleSvgString({shape:'round', x:0, y:0, w:1, h:1, text:''});
  // 최소 40×30 으로 보정 — 중심점 (20, 15), rx=20, ry=15
  assert.match(out, /cx="20"/);
  assert.match(out, /cy="15"/);
});

test('bubbleSvgString: null/빈 입력 안전', () => {
  assert.equal(bubbleSvgString(null), '');
  assert.equal(bubbleSvgString({}).startsWith('<g'), true);
});

test('serializeProject: 기본 직렬화', () => {
  const p = serializeProject({
    layoutId: '4grid',
    panels: [{prompt:'A'}, {prompt:'B'}, {prompt:'C'}, {prompt:'D'}],
    bubbles: [{shape:'round', x:10, y:20, w:100, h:60, text:'hi'}],
  });
  assert.equal(p.schemaVersion, 1);
  assert.equal(p.layoutId, '4grid');
  assert.equal(p.panels.length, 4);
  assert.equal(p.panels[1].prompt, 'B');
  assert.equal(p.bubbles[0].text, 'hi');
  assert.equal(p.bubbles[0].shape, 'round');
});

test('serializeProject: 잘못된 layoutId → null', () => {
  assert.equal(serializeProject({layoutId:'bad'}), null);
  assert.equal(serializeProject({}), null);
  assert.equal(serializeProject(null), null);
});

test('serializeProject: 길이 cap + layout panels cap', () => {
  const longText = 'x'.repeat(2000);
  const longPrompt = 'p'.repeat(8000);
  const p = serializeProject({
    layoutId: '2v',
    panels: [{prompt: longPrompt}, {prompt: longPrompt}, {prompt: 'extra'}],
    bubbles: Array.from({length: 200}, () => ({shape:'round', text: longText})),
  });
  assert.equal(p.panels.length, 2);          // layout 2v 는 2 패널
  assert.equal(p.panels[0].prompt.length, 4000);
  assert.equal(p.bubbles.length, 100);       // MAX_BUBBLES
  assert.equal(p.bubbles[0].text.length, 500);
});

test('serializeProject: 비정상 bubble 값 sanitize', () => {
  const p = serializeProject({
    layoutId: '2v',
    bubbles: [
      {shape:'evil', x:'abc', y:NaN, w:-50, h:'x', text:null, tailDir:'invalid'},
    ],
  });
  const b = p.bubbles[0];
  ...
  assert.equal(b.h, 80);
  assert.equal(b.text, '');
  assert.equal(b.tailDir, 'bl');
});

test('parseProject: schemaVersion 불일치 → null', () => {
  assert.equal(parseProject({schemaVersion: 99, layoutId: '2v'}), null);
  assert.equal(parseProject({layoutId: '2v'}), null);     // schemaVersion 없음
  assert.equal(parseProject(null), null);
  assert.equal(parseProject('x'), null);
});

test('parseProject: round-trip 보존', () => {
  const original = {
    layoutId: '4grid',
    panels: [{prompt:'A'}, {prompt:'B'}, {prompt:'C'}, {prompt:'D'}],
    bubbles: [{shape:'spike', x:5, y:10, w:80, h:50, text:'Bam!', tailDir:'br'}],
  };
  const s = serializeProject(original);
  const restored = parseProject(JSON.parse(JSON.stringify(s)));
  assert.equal(restored.layoutId, '4grid');
